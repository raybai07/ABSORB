...
##############################
## Non-bias corrected model ##
##############################
# Implements the model for bivariate meta-analysis without any correction
...
# init = initial values for (mu1, mu2, tau1, tau2, rho3, rho4)
...
BayesNonBiasCorrected <- function(y1, s1, y2, s2, seed=NULL, burn=10000, nmc=50000){
  ...
  if(m2!=0 & m3!=0){
    ...
    model_string <- "model{
      ...
      for(i in 1:m1){
        ...
        Sigma.both[i,1,1] <- pow(s.both[i,1],2)
        Sigma.both[i,1,2] <- rho3*s.both[i,1]*s.both[i,2]
        Sigma.both[i,2,1] <- rho3*s.both[i,1]*s.both[i,2]
        Sigma.both[i,2,2] <- pow(s.both[i,2],2)

        # Sample y1 and y2
        y.both[i,1:2] ~ dmnorm.vcov(theta.both[i,1:2], Sigma.both[i,1:2,1:2])
      }

      # For the studies that report first endpoint ONLY
      for(i in 1:m2){
        # Sample y1
        y1.only[i] ~ dnorm(theta1.only[i], 1/pow(s1.only[i],2))
      }

      # For the studies that report second endpoint ONLY
      for(i in 1:m3){
        # Sample y2
        y2.only[i] ~ dnorm(theta2.only[i], 1/pow(s2.only[i],2))
      }
    ...
    # Next use the function 'coda.samples' to produce the next 50,000
    ...
    model_string <- "model{
      ...
      # Prior for theta1 for studies that reported second endpoint ONLY
      for(i in 1:m2){
        theta1.only[i] ~ dnorm(mu1, 1/pow(tau1,2))
      }
  ...
  # Extract samples. Can be used for plotting and obtaining summary statistics.
